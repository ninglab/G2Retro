...
#from molcenter import MolCenter
from vocab_full import Vocab, common_atom_vocab
...
parser.add_argument('--use_cano', action="store_true")
...
model_center = MolCenter(vocab, common_atom_vocab, args)
try:
...
model_synthon = MolSynthon(vocab, common_atom_vocab, args)
try:
...
with open(args.test_path) as f:
    for i, line in enumerate(f.readlines()[1:]):
        s = line.strip("\r\n ").split(",")
        smiles = s[0].split(">>")
        data.append((0, "%d" % (i), smiles[1], smiles[0]))
...
loader = MolTreeFolder(data[start:end], vocab, avocab, args.batch_size, test=True, \
                       use_atomic=True, use_class=args.use_class, use_brics=args.use_brics, use_feature=True, del_center=True, usepair=False)
...
for batch in loader:
    ...
    with torch.no_grad():
        ...
        idxs = [None for _ in reacts_smiles]
        
        test_product_smiles = [tree.smiles for tree in product_tree]
        for i, trees in enumerate(top_k_trees):
            ...
            for j, tree in enumerate(trees):
                ...
                if is_sim(tree.smiles, synthon_smiles[j]): idxs[j] = i
        
        pre_react_smiles, pre_react_logs = model_synthon.test_synthon_beam_search(classes, product_batch, product_tree, top_k_trees, top_k_synthon_batch, buffer_log_probs, knum=10, tmp=test_product_smiles)
    ...
    for i, react_smile in enumerate(reacts_smiles):
        ...
        for j in range(len(pre_react_smiles[idx])):
            ...
            if is_sim(pre_smile, react_smile):
                ...
                print("%s match (%.2f)" % (product_smiles[idx][0], pre_react_logs[idx][j]))
                break
            else:
                string = "%s: %s fail to match %s with %s (%.2f)\n" % (product_smiles[idx][0], product_smiles[idx][1], react_smile, pre_smile, pre_react_logs[idx][j])
                print(string)
    ...
    for i, react_smile in enumerate(reacts_smiles):
        ...
        for j in range(len(pre_react_smiles[idx])):
            pre_smile = pre_react_smiles[idx][j]
            pred_file.write("%s %s %s %s\n" % (product_smiles[idx][0], product_smiles[idx][1], react_smile, pre_smile))
        idx += 1
